...
class VisionAgentSurrealist:
    """Agent for analyzing surrealist art using Gemini and Grok Vision APIs."""
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        if not google_api_key:
            logging.warning("VisionAgentSurrealist: WARNING: No GOOGLE_API_KEY found in config or environment variables. Gemini Pro will be unavailable. Analysis will proceed with other available models.")
            ...
            try:
                ...
                logging.info("VisionAgentSurrealist: Gemini Pro model initialized.")
            except Exception as e:
                logging.warning(f"VisionAgentSurrealist: Failed to initialize Gemini Pro model due to missing key or other issue: {e}. Analysis will proceed without Gemini Pro.")
                self.gemini_pro = None
        ...
        if not grok_api_key:
            logging.warning("VisionAgentSurrealist: WARNING: No GROK_API_KEY found in config or environment variables. Grok Vision will be unavailable. Analysis will proceed with other available models.")
            ...
            try:
                ...
                logging.info(f"VisionAgentSurrealist: Grok Vision client initialized with model {self.grok_vision_model}.")
            except Exception as e:
                logging.warning(f"VisionAgentSurrealist: Failed to initialize Grok Vision client due to missing key or other issue: {e}. Analysis will proceed without Grok Vision.")
                self.grok_client = None
    ...
    def analyze_image(self, image_path: str, research_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
        Analyzes surrealist image using Gemini Pro and Grok Vision,
        incorporating research data.
        """
        self._think_with_grok(
            f"Preparing to analyze surrealist image at {image_path} with research data: {research_data}. "
            "What are the key reasoning steps for optimal surrealist object and motif detection?"
        )
        try:
            # Store research data
            ...
            try:
                ...
                logging.error(f"VisionAgentSurrealist: Image file not found in analyze_image: {image_path}. Error: {e}")
                raise
            except UnidentifiedImageError as e:
                logging.error(f"VisionAgentSurrealist: Cannot identify image file (corrupted or unsupported format) in analyze_image: {image_path}. Error: {e}")
                raise CorruptedImageError(f"Corrupted or unsupported image file: {image_path}") from e
            except IOError as e:
                logging.error(f"VisionAgentSurrealist: IOError opening image in analyze_image: {image_path}. Error: {e}")
                ...
                logging.error(f"VisionAgentSurrealist: Failed to load or preprocess image: {e}")
            ...
            results = {
                ...
                "style": research_data.get("style", "surrealist"),
                "primary_subject": self.primary_subject,
                "secondary_subjects": self.secondary_subjects
            }

            # Surrealist-specific prompt focusing on unusual objects, juxtapositions, and symbolic elements
            surrealist_prompt = f"""
            Analyze this surrealist painting. Focus on identifying and localizing unusual or unexpected objects, juxtapositions, distorted forms, and symbolic elements. Describe the overall mood or narrative suggested by the scene.
            ...
            - Objects that appear out of place or are distorted
            - Juxtaposed objects or elements
            - Recognizable figures or objects in unusual contexts
            - Symbolic elements or motifs
            - Elements of the setting that contribute to the surreal atmosphere.

            For each identified object, provide:
            - label: Concise description (e.g., "melting clock", "floating eye", "figure with a drawer")
            - box_2d: [y0, x0, y1, x1]
            - confidence: 0.0-1.0
            - type: Category (e.g., "distorted_object", "symbol", "figure_in_context", "setting_element")
            ...
            if self.gemini_pro:
                try:
                    gemini_response = self.gemini_pro.generate_content(
                        contents=[surrealist_prompt, img],
                        generation_config=GenerationConfig(
                            temperature=0.6, # Higher temp to encourage identification of unusual elements
                            top_p=0.8,
                            top_k=40
                        )
                    )
                    ...
                    if gemini_results and "objects" in gemini_results:
                        ...
                        logging.info(f"VisionAgentSurrealist: Gemini Pro identified {len(gemini_objects)} objects.")
                except Exception as e:
                    logging.error(f"VisionAgentSurrealist: Error in Gemini Pro analysis: {e}")
            ...
            if self.grok_client:
                 try:
                    grok_response = self.grok_client.chat.completions.create(
                        messages=[
                            {"role": "user", "content": [{"type": "text", "text": surrealist_prompt}, {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{base64.b64encode(content).decode('utf-8')}"}}]}
                        ],
                    ...
                    if grok_results and "objects" in grok_results:
                        ...
                        logging.info(f"VisionAgentSurrealist: Grok Vision identified {len(grok_objects)} objects.")
                 except Exception as e:
                    logging.error(f"VisionAgentSurrealist: Error in Grok Vision analysis: {e}")


            # --- Result Aggregation ---
            combined_objects = self._aggregate_results(gemini_objects, grok_objects)
            results["objects"] = combined_objects
            logging.info(f"VisionAgentSurrealist: Aggregated {len(results['objects'])} objects from dual models.")
            ...
            for obj in results["objects"]:
                 if primary_subject_label and primary_subject_label in obj.get("label", "").lower():
                      ...
                      logging.info(f"VisionAgentSurrealist: Boosted primary subject '{obj['label']}' importance to {obj['importance']:.2f}")
        ...
        except (CorruptedImageError, UnsupportedImageFormatError, FileNotFoundError, RuntimeError) as e:
            # Re-raise known exceptions that should halt processing
            raise
        except Exception as e: # Catch any other unexpected exceptions
            logging.exception(f"VisionAgentSurrealist: Unexpected error in analyze_image: {e}")
    ...
    def _calculate_object_importance(self, box_2d: List[float], obj_type: str, img_size: Tuple[int, int], base_importance: float, label: str, primary_subject: Optional[str], secondary_subjects: List[str]) -> float:
        """Calculate scaled importance score for surrealist art."""
        ...
        # Surrealist-specific multipliers (emphasizing unusual/symbolic elements)
        type_modifiers = {
            'distorted_object': 3.5,
            'symbol': 3.8,
            'figure_in_context': 3.0, # Figure in an unusual setting
            'object': 2.5, # General objects might have symbolic meaning
            'setting_element': 1.5, # Setting contributes to the mood
            'background': 0.8, # Background can be important for atmosphere
            'unknown': 2.0, # Give unknown objects higher potential importance
        }
        ...
        # Scale by coverage (prominent surreal elements are important)
    ...
    def _threshold_important_objects(self, objects: List[Dict[str, Any]], percentile_threshold: float = 70) -> List[Dict[str, Any]]:
        """Filter objects for surrealist art - lower threshold, prioritize unusual/symbolic elements and subjects."""
        if not objects:
            return []
        ...
        # Lower percentile threshold for surrealist art to capture more potentially significant elements
        ...
        for obj in objects:
            ...
            if obj.get('importance', 0) >= threshold or \
               obj.get('type') in ['distorted_object', 'symbol', 'figure_in_context', 'object'] or \
    ...
    def save_analysis_outputs(self, image_path: str, analysis_results: Dict[str, Any], output_folder: str) -> None:
        ...
        try:
            ...
            logging.info(f"VisionAgentSurrealist: Saved all analysis outputs for {basename}")

            # TODO: Handoff cropped_path to UpscaleAgent queue

        except Exception as e:
            logging.exception(f"VisionAgentSurrealist: Error saving analysis outputs: {e}")
    ...
    def _save_masked_version(self, img: Image.Image, analysis_results: Dict[str, Any], output_path: str) -> None: # Corrected parameter name
        ...
        try:
            ...
            os.makedirs(os.path.dirname(output_path), exist_ok=True)
            img.save(output_path, format='JPEG', quality=95)
            logging.info(f"VisionAgentSurrealist: Saved masked image to {output_path}")

        except Exception as e:
            logging.exception(f"VisionAgentSurrealist: Error saving masked version to {output_path}: {e}")
    ...
    def copy_and_crop_image(self, input_path: str, output_path: str, analysis_results: Dict[str, Any]) -> str:
        ...
        try:
            ...
            try:
                ...
                error_msg = f"Cannot identify image file (corrupted or unsupported format) for surrealist: {input_path}. Error: {e}"
                logging.error(f"VisionAgentSurrealist: {error_msg}")
                raise CorruptedImageError(error_msg) from e
            except FileNotFoundError as e:
                error_msg = f"Input file not found for surrealist cropping: {input_path}. Error: {e}"
                logging.error(f"VisionAgentSurrealist: {error_msg}")
                raise
            except IOError as e:
                error_msg = f"IOError opening image for surrealist cropping (possibly unsupported format): {input_path}. Error: {e}"
                logging.error(f"VisionAgentSurrealist: {error_msg}")
            ...
            if new_width <= 0 or new_height <= 0:
                error_msg = f"Invalid target dimensions calculated for surrealist crop: {new_width}x{new_height}"
                logging.error(f"VisionAgentSurrealist: {error_msg}")
            ...
            if best_crop:
                ...
                logging.info(f"VisionAgentSurrealist: Saved cropped image to {output_path}")
                return output_path
            else:
                # Fallback to center crop
                logging.warning("VisionAgentSurrealist: Could not determine optimal crop. Using center crop as fallback.")
                ...
                if left < 0 or top < 0 or right > width or bottom > height or new_width <= 0 or new_height <= 0:
                    error_msg = f"Invalid center crop dimensions for surrealist: L{left} T{top} R{right} B{bottom} for image {width}x{height}. Saving original."
                    logging.error(f"VisionAgentSurrealist: {error_msg}")
                ...
                logging.info(f"VisionAgentSurrealist: Saved center-cropped image to {output_path} as fallback.")
                return output_path
            ...
            error_msg = f"IOError during surrealist image cropping/saving: {input_path}. Error: {e}"
            logging.error(f"VisionAgentSurrealist: {error_msg}")
            raise UnsupportedImageFormatError(error_msg) from e
        except Exception as e: # General fallback
            error_msg = f"Unexpected error cropping surrealist image {input_path}: {e}"
            logging.exception(f"VisionAgentSurrealist: {error_msg}")
    ...
    def _think_with_grok(self, prompt: str) -> Optional[str]:
        ...
        if not self.grok_client:
            logging.warning("VisionAgentSurrealist: Grok client unavailable for internal reasoning.")
            return None
        try:
            ...
            logging.info(f"VisionAgentSurrealist: Grok thinking output: {reasoning}")
            return reasoning
        except Exception as e:
            logging.error(f"VisionAgentSurrealist: Grok thinking failed: {e}")
            return None
