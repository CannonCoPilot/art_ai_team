...
class VisionAgentPortrait:
    """Agent for analyzing portrait art using Gemini and Grok Vision APIs."""
    ...
    def analyze_image(self, image_path: str, research_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            results = {
                ...
                "style": research_data.get("style", "portrait"),
                "primary_subject": self.primary_subject,
                "secondary_subjects": self.secondary_subjects
            }

            # Portrait-specific prompt focusing on faces, heads, and features
            portrait_prompt = f"""
            ...
            - Important objects (jewelry, clothing details, props)
            - Background elements (less important)
            ...
            if self.gemini_pro:
                try:
                    gemini_response = self.gemini_pro.generate_content(
                        contents=[portrait_prompt, img],
                        generation_config=GenerationConfig(
                            temperature=0.3, # Focus on accurate detection
                            top_p=0.8,
                            top_k=40
                        )
                    )
            ...
            if self.grok_client:
                 try:
                    grok_response = self.grok_client.chat.completions.create(
                        messages=[
                            {"role": "user", "content": [{"type": "text", "text": portrait_prompt}, {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{base64.b64encode(content).decode('utf-8')}"}}]}
                        ],
    ...
    def _calculate_object_importance(self, box_2d: List[float], obj_type: str, img_size: Tuple[int, int], base_importance: float, label: str, primary_subject: Optional[str], secondary_subjects: List[str], features: List[Dict]) -> float:
        """Calculate scaled importance score for portrait art."""
        ...
        # Portrait-specific multipliers
        type_modifiers = {
            'human_face': 4.0, # Highest priority for the face
            'human_figure': 3.5, # Full figure is also very important
            'facial_feature': 3.8, # Individual features are highly important
            'animal_face': 2.0, # Secondary animal faces
            'animal_figure': 1.8, # Secondary animal figures
            'object': 1.5, # Important objects (jewelry, props)
            'background': 0.3, # Heavily discount background
            'unknown': 1.0,
        }
        ...
        # Scale by coverage (face/head coverage is very important)
    ...
    def _threshold_important_objects(self, objects: List[Dict[str, Any]], percentile_threshold: float = 90) -> List[Dict[str, Any]]:
        """Filter objects for portrait art - prioritize faces, figures, and subjects."""
        if not objects:
            return []
        ...
        # Higher percentile threshold for portraits to focus on the main subject
        ...
        for obj in objects:
            ...
            if obj.get('importance', 0) >= threshold or \
               obj.get('type') in ['human_face', 'human_figure', 'facial_feature', 'animal_face', 'animal_figure'] or \
