...
class VisionAgentReligiousHistorical:
    """Agent for analyzing religious/historical art using Gemini and Grok Vision APIs."""

    def __init__(self, config: Optional[Dict[str, Any]] = None):
        """Initialize the VisionAgentReligiousHistorical with configuration."""
        ...
        google_api_key = self.config.get('google_api_key') if self.config and 'google_api_key' in self.config else os.environ.get('GOOGLE_API_KEY')
        if not google_api_key:
            logging.warning("VisionAgentReligiousHistorical: WARNING: No GOOGLE_API_KEY found in config or environment variables. Gemini Pro will be unavailable. Analysis will proceed with other available models.")
            ...
            try:
                ...
                logging.info("VisionAgentReligiousHistorical: Gemini Pro model initialized.")
            except Exception as e:
                logging.warning(f"VisionAgentReligiousHistorical: Failed to initialize Gemini Pro model due to missing key or other issue: {e}. Analysis will proceed without Gemini Pro.")
                self.gemini_pro = None

        # Initialize Grok Vision model
        grok_api_key = self.config.get('grok_api_key') if self.config and 'grok_api_key' in self.config else os.environ.get('GROK_API_KEY')
        if not grok_api_key:
            logging.warning("VisionAgentReligiousHistorical: WARNING: No GROK_API_KEY found in config or environment variables. Grok Vision will be unavailable. Analysis will proceed with other available models.")
            ...
            try:
                ...
                logging.warning(f"VisionAgentReligiousHistorical: Failed to initialize Grok Vision client due to missing key or other issue: {e}. Analysis will proceed without Grok Vision.")
                self.grok_client = None
    ...
    def analyze_image(self, image_path: str, research_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
        Analyzes religious/historical image using Gemini Pro and Grok Vision,
        incorporating research data.
        """
        self._think_with_grok(
            f"Preparing to analyze religious/historical image at {image_path} with research data: {research_data}. "
            "What are the key reasoning steps for optimal object and symbol detection?"
        )
        try:
            if not isinstance(research_data, dict):
                error_msg = f"research_data must be a dict, got {type(research_data)}"
                logging.error(f"VisionAgentReligiousHistorical: {error_msg}")
            ...
            try:
                ...
                logging.error(f"VisionAgentReligiousHistorical: Cannot identify image file (corrupted or unsupported format) in analyze_image: {image_path}. Error: {e}")
                ...
                logging.error(f"VisionAgentReligiousHistorical: IOError opening image in analyze_image: {image_path}. Error: {e}")
                ...
                logging.error(f"VisionAgentReligiousHistorical: Failed to load or preprocess image: {e}")
            ...
            results = {
                ...
                "style": research_data.get("style", "religious/historical"),
                "primary_subject": self.primary_subject,
                "secondary_subjects": self.secondary_subjects
            }

            # Religious/Historical-specific prompt focusing on figures, symbols, and narrative elements
            religious_historical_prompt = f"""
            Analyze this religious or historical painting. Focus on identifying and localizing key figures, religious or historical symbols, and elements that convey the narrative or context. Pay attention to gestures, expressions, and interactions.
            ...
            - Central figures (religious figures, historical persons)
            - Secondary figures and groups
            - Religious symbols (crosses, halos, specific objects)
            - Historical artifacts or clothing
            - Elements of the setting that indicate time period or location
            - Objects that are part of the narrative or convey meaning

            For each identified object, provide:
            - label: Concise description (e.g., "Jesus Christ", "Virgin Mary", "Roman soldier", "chalice", "ancient scroll")
            - box_2d: [y0, x0, y1, x1]
            - confidence: 0.0-1.0
            - type: Category (e.g., "religious_figure", "historical_figure", "symbol", "artifact", "setting_element", "group")
            - importance: Initial estimate 0.0-1.0 (will be refined by agent based on perceived significance)
            ...
            if self.gemini_pro:
                try:
                    gemini_response = self.gemini_pro.generate_content(
                        contents=[religious_historical_prompt, img],
                    ...
                    if gemini_results and "objects" in gemini_results:
                        ...
                        logging.info(f"VisionAgentReligiousHistorical: Gemini Pro identified {len(gemini_objects)} objects.")
                except Exception as e:
                    logging.error(f"VisionAgentReligiousHistorical: Error in Gemini Pro analysis: {e}")
            ...
            logging.info(f"VisionAgentReligiousHistorical: Aggregated {len(results['objects'])} objects from dual models.")
            ...
            for obj in results["objects"]:
                 if primary_subject_label and primary_subject_label in obj.get("label", "").lower():
                      ...
                      logging.info(f"VisionAgentReligiousHistorical: Boosted primary subject '{obj['label']}' importance to {obj['importance']:.2f}")
            ...
            logging.exception(f"VisionAgentReligiousHistorical: Unexpected error in analyze_image: {e}")
    ...
    def _calculate_object_importance(self, box_2d: List[float], obj_type: str, img_size: Tuple[int, int], base_importance: float, label: str, primary_subject: Optional[str], secondary_subjects: List[str], features: List[Dict]) -> float:
        """Calculate scaled importance score for religious/historical art."""
        ...
        # Religious/Historical-specific multipliers
        type_modifiers = {
            'religious_figure': 4.0, # Highest priority for key religious figures
            'historical_figure': 3.8, # High priority for key historical figures
            'symbol': 3.5, # Important symbols
            'artifact': 3.0, # Historical artifacts
            'figure': 2.5, # Other figures
            'group': 2.8, # Groups of figures
            'setting_element': 1.5, # Setting elements relevant to context
            'object': 1.8, # Other relevant objects
        ...
        if primary_subject_lower and (primary_subject_lower in label_lower or obj_type_lower in ['religious_figure', 'historical_figure', 'figure', 'group']):
             style_based_score *= 2.5 # Strong boost for primary subject and related figures
        elif any(sub_lower in label_lower for sub_lower in secondary_subject_lowers) or obj_type_lower in ['religious_figure', 'historical_figure', 'figure', 'group']:
             style_based_score *= 1.5 # Moderate boost for secondary subjects and related figures

        # Add bonus for detected facial features (especially for key figures)
        if features and (obj_type_lower in ['religious_figure', 'historical_figure', 'figure']):
             style_based_score *= (1 + len(features) * 0.15) # Higher bonus for facial features on figures

        total_score = max(base_importance, style_based_score)

        # Scale by coverage (central figures/symbols are important)
        coverage_factor = np.sqrt(coverage_ratio)
        final_score = total_score * (0.5 + 0.5 * coverage_factor) # Higher base impact, less coverage scaling
    ...
    def save_analysis_outputs(self, image_path: str, analysis_results: Dict[str, Any], output_folder: str) -> None:
        ...
        try:
            ...
            logging.info(f"VisionAgentReligiousHistorical: Saved all analysis outputs for {basename}")

            # TODO: Handoff cropped_path to UpscaleAgent queue

        except Exception as e:
            logging.exception(f"VisionAgentReligiousHistorical: Error saving analysis outputs: {e}")
        ...
        try:
            ...
            try:
                if not os.path.exists(input_path): # Keep the explicit check for FileNotFoundError
                    error_msg = f"Input image file not found for religious/historical cropping: {input_path}"
                    logging.warning(f"[VisionAgentReligiousHistorical] {error_msg}")
                    raise FileNotFoundError(error_msg)
                img = Image.open(input_path)
            except UnidentifiedImageError as e:
                error_msg = f"Cannot identify image file (corrupted or unsupported format) for religious/historical: {input_path}. Error: {e}"
                logging.error(f"[VisionAgentReligiousHistorical] {error_msg}")
                raise CorruptedImageError(error_msg) from e
            except FileNotFoundError: # Re-raise from the os.path.exists check
                raise
            except IOError as e: # Catch other IOErrors that might indicate unsupported formats
                error_msg = f"IOError opening image for religious/historical cropping (possibly unsupported format): {input_path}. Error: {e}"
                logging.error(f"[VisionAgentReligiousHistorical] {error_msg}")
            ...
            if new_width <= 0 or new_height <= 0:
                error_msg = f"Invalid target dimensions calculated for religious/historical crop: {new_width}x{new_height}"
                logging.error(f"[VisionAgentReligiousHistorical] {error_msg}")
            ...
            if best_crop:
                ...
                logging.info(f"[VisionAgentReligiousHistorical] Saved cropped image to {output_path}")
                return output_path
            else:
                # Fallback to center crop
                logging.warning("[VisionAgentReligiousHistorical] Could not determine optimal crop. Using center crop as fallback.")
                ...
                if left < 0 or top < 0 or right > width or bottom > height or new_width <= 0 or new_height <= 0:
                    error_msg = f"Invalid center crop dimensions for religious/historical: L{left} T{top} R{right} B{bottom} for image {width}x{height}. Saving original."
                    logging.error(f"[VisionAgentReligiousHistorical] {error_msg}")
                ...
                logging.info(f"[VisionAgentReligiousHistorical] Saved center-cropped image to {output_path} as fallback.")
                return output_path
            ...
            error_msg = f"IOError during religious/historical image cropping/saving: {input_path}. Error: {e}"
            logging.error(f"[VisionAgentReligiousHistorical] {error_msg}")
            raise UnsupportedImageFormatError(error_msg) from e
        except Exception as e: # General fallback
            error_msg = f"Unexpected error cropping religious/historical image {input_path}: {e}"
            logging.exception(f"[VisionAgentReligiousHistorical] {error_msg}")
    ...
    def _find_optimal_crop(self, img_size: Tuple[int, int], target_size: Tuple[int, int],
        ...
        # Religious/Historical-specific cropping refinement (Roadmap Step 7)
        if best_crop:
             best_crop = self._refine_religious_historical_crop(best_crop, img_size, target_size, pixel_objects, masks, self.primary_subject, self.secondary_subjects)


        return best_crop


    def _refine_religious_historical_crop(self, current_crop: Tuple[int, int, int, int], img_size: Tuple[int, int], target_size: Tuple[int, int], objects: List[Dict[str, Any]], masks: List[SegmentationMask], primary_subject: Optional[str], secondary_subjects: List[str]) -> Tuple[int, int, int, int]:
        """
        Refines the crop for religious/historical paintings based on mask avoidance and subject priority (similar to landscape).
        (Roadmap Step 7 - Religious/Historical specific)
        """
        ...
        # Handle primary subject truncation (Roadmap Step 7 - Religious/Historical specific, follow Landscape)
        ...
        if primary_subject_mask:
             ...
             if is_truncated_top or is_truncated_bottom or is_truncated_left or is_truncated_right:
                  logging.warning(f"VisionAgentReligiousHistorical: Primary subject '{primary_subject}' is truncated in the crop.")

             # Religious/Historical specific rule: follow Landscape truncation logic
             # Truncate from bottom up if necessary in y-axis
             # Maintain centrality or offset in x-axis
             pass # Placeholder for religious/historical specific refinement logic
    ...
    def _evaluate_crop(self, crop_box: Tuple[int, int, int, int],
        ...
        for obj in objects:
            ...
            if intersect_right > intersect_left and intersect_bottom > intersect_top:
                ...
                # Adjust importance based on object type and context (Religious/Historical specific)
                # These multipliers should ideally be defined in the VisionAgentReligiousHistorical class
                # For now, using simplified logic based on type/subject status
                adjusted_importance = importance

                if obj_type.lower() in ['religious_figure', 'historical_figure', 'symbol', 'artifact']:
                     adjusted_importance *= 3.0 # High boost for key elements
                elif obj_type.lower() in ['figure', 'group', 'setting_element', 'object']:
                     adjusted_importance *= 1.8 # Moderate boost for other relevant elements

                if is_primary_subject:
                     adjusted_importance *= 2.5 # Additional boost for primary subject
                elif is_secondary_subject:
                     adjusted_importance *= 1.5 # Additional boost for secondary subjects


                # Penalize partial coverage of important objects
                if coverage < 1.0:
                    if is_primary_subject or is_secondary_subject or obj_type.lower() in ['religious_figure', 'historical_figure', 'symbol', 'artifact', 'figure', 'group']:
                        adjusted_importance *= coverage * 0.5  # Severe penalty for important elements
                    else:
                        adjusted_importance *= coverage * 0.8  # Moderate penalty for others
    ...
    def _think_with_grok(self, prompt: str) -> Optional[str]:
        ...
        if not self.grok_client:
            logging.warning("VisionAgentReligiousHistorical: Grok client unavailable for internal reasoning.")
            return None
        try:
            ...
            logging.info(f"VisionAgentReligiousHistorical: Grok thinking output: {reasoning}")
            return reasoning
        except Exception as e:
            logging.error(f"VisionAgentReligiousHistorical: Grok thinking failed: {e}")
            return None
    ...
    def handoff_to_upscale(self, cropped_image_path: str, upscale_queue: Queue):
        ...
        logging.info(f"VisionAgentReligiousHistorical: Handoff {os.path.basename(cropped_image_path)} to UpscaleAgent queue.")
