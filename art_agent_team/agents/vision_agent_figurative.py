...
class VisionAgentFigurative:
    """Agent for analyzing figurative art using Gemini and Grok Vision APIs."""
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        if not google_api_key:
            logging.warning("VisionAgentFigurative: WARNING: No GOOGLE_API_KEY found in config or environment variables. Gemini Pro will be unavailable. Analysis will proceed with other available models.")
            ...
            try:
                ...
                logging.info("VisionAgentFigurative: Gemini Pro model initialized.")
            except Exception as e:
                logging.warning(f"VisionAgentFigurative: Failed to initialize Gemini Pro model due to missing key or other issue: {e}. Analysis will proceed without Gemini Pro.")
                self.gemini_pro = None

        grok_api_key = config.get('grok_api_key') if config and 'grok_api_key' in config else os.environ.get('GROK_API_KEY')
        if not grok_api_key:
            logging.warning("VisionAgentFigurative: WARNING: No GROK_API_KEY found in config or environment variables. Grok Vision will be unavailable. Analysis will proceed with other available models.")
            ...
            try:
                ...
                logging.info(f"VisionAgentFigurative: Grok Vision client initialized with model {self.grok_vision_model}.")
            except Exception as e:
                logging.warning(f"VisionAgentFigurative: Failed to initialize Grok Vision client due to missing key or other issue: {e}. Analysis will proceed without Grok Vision.")
                self.grok_client = None
    ...
    def analyze_image(self, image_path: str, research_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
        Analyzes figurative image using Gemini Pro and Grok Vision,
        incorporating research data.
        """
        self._think_with_grok(
            f"Preparing to analyze figurative image at {image_path} with research data: {research_data}. "
            "What are the key reasoning steps for optimal detection of figures and interactions?"
        )
        try:
            # Store research data
            ...
            try:
                ...
                logging.error(f"VisionAgentFigurative: Cannot identify image file (corrupted or unsupported format) in analyze_image: {image_path}. Error: {e}")
                raise CorruptedImageError(f"Corrupted or unsupported image file: {image_path}") from e
            except FileNotFoundError as e:
                logging.error(f"VisionAgentFigurative: Image file not found in analyze_image: {image_path}. Error: {e}")
                raise
            except IOError as e:
                logging.error(f"VisionAgentFigurative: IOError opening image in analyze_image: {image_path}. Error: {e}")
            ...
            results = {
                ...
                "style": research_data.get("style", "figurative"),
                "primary_subject": self.primary_subject,
                "secondary_subjects": self.secondary_subjects
            }

            # Figurative-specific prompt focusing on figures, faces, and actions
            figurative_prompt = f"""
            Analyze this figurative painting. Focus on identifying and localizing all human and animal figures, especially their faces and key features. Note their poses, actions, and interactions.
            ...
            - All human figures (full bodies)
            - All animal figures (full bodies)
            - Human faces/heads (eyes, nose, mouth, ears, hair, facial hair)
            - Animal faces/heads (beaks, snouts, muzzles, eyes, ears, manes, crests)
            - Actions, poses, and interactions between figures
            - Important objects held by or near figures

            For each identified object, provide:
            - label: Concise description (e.g., "man standing", "woman sitting", "dog barking", "two figures talking")
            - box_2d: [y0, x0, y1, x1]
            - confidence: 0.0-1.0
            - type: Category (e.g., "human_figure", "animal_figure", "human_face", "animal_face", "interaction", "object")
            ...
            if self.gemini_pro:
                try:
                    gemini_response = self.gemini_pro.generate_content(
                        contents=[figurative_prompt, img],
                    ...
                    if gemini_results and "objects" in gemini_results:
                        ...
                        logging.info(f"VisionAgentFigurative: Gemini Pro identified {len(gemini_objects)} objects.")
                except Exception as e:
                    logging.error(f"VisionAgentFigurative: Error in Gemini Pro analysis: {e}")
            ...
            if self.grok_client:
                 try:
                    grok_response = self.grok_client.chat.completions.create(
                        messages=[
                            {"role": "user", "content": [{"type": "text", "text": figurative_prompt}, {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{base64.b64encode(content).decode('utf-8')}"}}]}
                        ],
                    ...
                    if grok_results and "objects" in grok_results:
                        ...
                        logging.info(f"VisionAgentFigurative: Grok Vision identified {len(grok_objects)} objects.")
                 except Exception as e:
                    logging.error(f"VisionAgentFigurative: Error in Grok Vision analysis: {e}")


            # --- Result Aggregation ---
            combined_objects = self._aggregate_results(gemini_objects, grok_objects)
            results["objects"] = combined_objects
            logging.info(f"VisionAgentFigurative: Aggregated {len(results['objects'])} objects from dual models.")
            ...
            for obj in results["objects"]:
                 if primary_subject_label and primary_subject_label in obj.get("label", "").lower():
                      ...
                      logging.info(f"VisionAgentFigurative: Boosted primary subject '{obj['label']}' importance to {obj['importance']:.2f}")
            ...
            logging.exception(f"VisionAgentFigurative: Error in analyze_image: {e}")
            return None
    ...
    def _calculate_object_importance(self, box_2d: List[float], obj_type: str, img_size: Tuple[int, int], base_importance: float, label: str, primary_subject: Optional[str], secondary_subjects: List[str], features: List[Dict]) -> float:
        """Calculate scaled importance score for figurative art."""
        ...
        # Figurative-specific multipliers
        type_modifiers = {
            'human_figure': 3.0,
            'animal_figure': 2.5,
            'human_face': 3.5, # Faces are very important in figurative art
            'animal_face': 3.0,
            'interaction': 2.8,
            'object': 1.5, # Objects held or near figures are important
        ...
        if features:
             ...
             if is_primary_subject or obj_type_lower in ['human_face', 'human_figure']:
                  feature_bonus_factor = 0.2 # Higher bonus for primary subject's features
             style_based_score *= (1 + len(features) * feature_bonus_factor)


        total_score = max(base_importance, style_based_score)

        # Scale by coverage (larger figures are more important)
    ...
    def _threshold_important_objects(self, objects: List[Dict[str, Any]], percentile_threshold: float = 85) -> List[Dict[str, Any]]:
        """Filter objects for figurative art - prioritize figures, faces, and subjects."""
        if not objects:
            return []
        ...
        # Higher percentile threshold for figurative art to focus on the main subject
        ...
        for obj in objects:
            ...
            if obj.get('importance', 0) >= threshold or \
               obj.get('type') in ['human_figure', 'animal_figure', 'human_face', 'animal_face', 'facial_feature'] or \
    ...
    def save_analysis_outputs(self, image_path: str, analysis_results: Dict[str, Any], output_folder: str) -> None:
        ...
        try:
            ...
            logging.info(f"VisionAgentFigurative: Saved all analysis outputs for {basename}")

            # TODO: Handoff cropped_path to UpscaleAgent queue

        except Exception as e:
            logging.exception(f"VisionAgentFigurative: Error saving analysis outputs: {e}")
    ...
    def copy_and_crop_image(self, input_path: str, output_path: str, analysis_results: Dict[str, Any]) -> str:
        ...
        try:
            ...
            try:
                ...
                error_msg = f"Cannot identify image file (corrupted or unsupported format) for figurative: {input_path}. Error: {e}"
                logging.error(f"[VisionAgentFigurative] {error_msg}")
                raise CorruptedImageError(error_msg) from e
            except FileNotFoundError as e:
                error_msg = f"Input file not found for figurative cropping: {input_path}. Error: {e}"
                logging.error(f"[VisionAgentFigurative] {error_msg}")
                raise
            except IOError as e:
                error_msg = f"IOError opening image for figurative cropping (possibly unsupported format): {input_path}. Error: {e}"
                logging.error(f"[VisionAgentFigurative] {error_msg}")
            ...
            if new_width <= 0 or new_height <= 0:
                error_msg = f"Invalid target dimensions calculated for figurative crop: {new_width}x{new_height}"
                logging.error(f"[VisionAgentFigurative] {error_msg}")
            ...
            if best_crop:
                ...
                logging.info(f"[VisionAgentFigurative] Saved cropped image to {output_path}")
                return output_path
            else:
                # Fallback to center crop
                logging.warning("[VisionAgentFigurative] Could not determine optimal crop. Using center crop as fallback.")
                ...
                if left < 0 or top < 0 or right > width or bottom > height or new_width <= 0 or new_height <= 0:
                    error_msg = f"Invalid center crop dimensions for figurative: L{left} T{top} R{right} B{bottom} for image {width}x{height}. Saving original."
                    logging.error(f"[VisionAgentFigurative] {error_msg}")
                ...
                logging.info(f"[VisionAgentFigurative] Saved center-cropped image to {output_path} as fallback.")
                return output_path
            ...
            error_msg = f"IOError during figurative image cropping/saving: {input_path}. Error: {e}"
            logging.error(f"[VisionAgentFigurative] {error_msg}")
            raise UnsupportedImageFormatError(error_msg) from e
        except Exception as e: # General fallback
            error_msg = f"Unexpected error cropping figurative image {input_path}: {e}"
            logging.exception(f"[VisionAgentFigurative] {error_msg}")
    ...
    def _think_with_grok(self, prompt: str) -> Optional[str]:
        ...
        if not self.grok_client:
            logging.warning("VisionAgentFigurative: Grok client unavailable for internal reasoning.")
            return None
        try:
            ...
            logging.info(f"VisionAgentFigurative: Grok thinking output: {reasoning}")
            return reasoning
        except Exception as e:
            logging.error(f"VisionAgentFigurative: Grok thinking failed: {e}")
            return None
